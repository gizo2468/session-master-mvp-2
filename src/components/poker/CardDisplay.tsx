    ...
    for (let i = 0; i < cards.length; i += 2) {
      if (i + 1 < cards.length) {
        ...
        cardArray.push(
          <div 
            key={`${rank}${suit}-${i}`}
            className={`${styles.card} inline-flex bg-white dark:bg-gray-50 dark:bg-background border border-gray-400 rounded-lg shadow-sm flex-col items-center justify-center gap-0.5 p-1 transition-transform hover:scale-105`}
          >
