...
5. CARD NOTATION REFERENCE:
   ...
   Detection Method (Auto-Selected):
   - Traditional Symbol Deck: Detect from suit symbols (♥♦♠♣)
   - Four-Color Deck: Detect from card BACKGROUND color (red/blue/green/black)
   
   Example outputs:
   - Traditional: Card with ♣ symbol → { rank: "Q", suit: "c", confidence: 0.9 }
   - Four-color: White "A" on blue background → { rank: "A", suit: "d", confidence: 0.85 }
   - Four-color: White "9" on green background → { rank: "9", suit: "c", confidence: 0.8 }

6. DECK TYPE AUTO-DETECTION (CRITICAL FIRST STEP):

   Before detecting suits, you MUST first determine which deck style is being used:
   
   DECK TYPE A - TRADITIONAL SYMBOL DECK (Standard):
   - Cards show small suit symbols (♥♦♠♣) in corners or centers
   - Card backgrounds are typically WHITE, CREAM, or LIGHT GRAY
   - Suits are indicated by SYMBOLS only, not by background color
   - Example: White card with "A" and small red ♥ symbol = Ace of Hearts
   
   DECK TYPE B - FOUR-COLOR DECK (GGPoker-style):
   - Cards have SOLID COLOR BACKGROUNDS filling the entire card rectangle
   - NO traditional suit symbols visible (or symbols are very small/unclear)
   - The CARD BACKGROUND COLOR itself indicates the suit:
     * RED background = Hearts
     * BLUE background = Diamonds
     * GREEN background = Clubs
     * BLACK background = Spades
   - Card rank (A, K, Q, etc.) is displayed in WHITE text over the colored background
   - Example: White "A" on blue background = Ace of Diamonds
   
   DETECTION STRATEGY:
   1. Look at 2-3 visible cards (hero cards or board cards)
   2. Check if cards have SOLID COLOR BACKGROUNDS:
      - If YES → Four-color deck (Type B) → Use COLOR DETECTION METHOD
      - If NO (white/light backgrounds) → Traditional deck (Type A) → Use SYMBOL DETECTION METHOD
   3. Log the detected deck type in metadata for debugging
   
   CONFIDENCE INDICATORS:
   - Strong color fill visible on multiple cards → Four-color deck (confidence 0.9+)
   - White ranks on colored backgrounds → Four-color deck (confidence 0.85+)
   - Traditional symbols clearly visible → Standard deck (confidence 0.9+)
   - Mixed signals or unclear → Use both methods, prefer higher confidence result

7. SUIT DETECTION - DUAL MODE SYSTEM:

   MODE A - SYMBOL DETECTION (for Traditional Decks):
   - Look for suit symbols (♥♦♠♣) on the card face
   - These appear as small icons in corners or center
   - Detection priority: Symbols are primary indicator
   - Output: h, d, s, c based on symbol shape
   - Confidence: 0.9+ if symbol clearly visible
   
   MODE B - COLOR DETECTION (for Four-Color Decks):
   
   **CRITICAL: This mode is used IMMEDIATELY when four-color deck is detected, NOT as a fallback**
   
   COLOR → SUIT MAPPING (Simplified):
   
   RED BACKGROUND → HEARTS (h):
   - Dominant color: Red tones (crimson, scarlet, rose)
   - Visual test: Does the card look predominantly RED?
   - Color family: Warm reds (avoid orange or purple confusion)
   - Relative comparison: Redder than blue, green, or black cards
   
   BLUE BACKGROUND → DIAMONDS (d):
   - Dominant color: Blue tones (royal blue, azure, navy)
   - Visual test: Does the card look predominantly BLUE?
   - Color family: Cool blues (can be quite dark but still blue-tinted)
   - Relative comparison: Bluer than red, green, or black cards
   - NOTE: Dark blue ≠ black (blues retain color tint, blacks don't)
   
   GREEN BACKGROUND → CLUBS (c):
   - Dominant color: Green tones (forest green, emerald, lime)
   - Visual test: Does the card look predominantly GREEN?
   - Color family: True greens (not yellow-green, not blue-green)
   - Relative comparison: Greener than all other cards
   - KEY DISTINCTION: Even DARK green retains visible greenish tint
   
   BLACK BACKGROUND → SPADES (s):
   - Dominant color: Black, dark gray, charcoal
   - Visual test: Does the card look GRAYSCALE (no color tint)?
   - Color family: Pure blacks and grays (zero color intensity)
   - Relative comparison: Darkest cards with NO color tint at all
   - KEY DISTINCTION: Black = ABSENCE of color, not just dark color
   
   SIMPLIFIED COLOR DETECTION STRATEGY:
   
   Step 1 - Sample Card Background:
   - Focus on the CENTER 60% of the card rectangle
   - Ignore white rank letters/numbers (they're foreground)
   - Extract the dominant background color
   
   Step 2 - Check for Color vs. Grayscale:
   - Does the card have ANY COLOR TINT at all?
     * YES (has color) → RED, BLUE, or GREEN (proceed to Step 3)
     * NO (pure gray/black) → BLACK → SPADES
   
   Step 3 - Identify Color Family (for colored cards):
   - Compare the card to RED, BLUE, and GREEN:
     * Which color family does it belong to?
     * What's the DOMINANT color tone?
     * Use relative comparison: Is it more red than blue? More green than red?
   
   Step 4 - Validate with Brightness:
   - RED and BLUE: Typically medium brightness (not too dark)
   - GREEN: Can be quite DARK but still shows green tint
   - BLACK: Very dark AND no color tint
   
   CRITICAL BLACK vs. GREEN DISAMBIGUATION:
   
   The #1 confusion is between BLACK SPADES and DARK GREEN CLUBS.
   Use this SIMPLE TWO-SIGNAL TEST:
   
   Signal 1 - Color Presence Test:
   - Look at the card background in good lighting
   - Does it have ANY greenish tint or color at all?
     * YES, even faint → GREEN → CLUBS
     * NO, pure gray/black → BLACK → SPADES
   
   Signal 2 - Comparison Test:
   - Compare this card to OTHER cards in the image
   - Is it the ONLY card with a greenish tint?
   - Are there OTHER black/gray cards that look identical?
     * If unique greenish card → CLUBS
     * If matches other pure black cards → SPADES
   
   RULE: When in doubt between black and green, check BOTH signals:
   - If EITHER signal indicates green → It's CLUBS (green)
   - Only if BOTH signals show NO color → It's SPADES (black)
   
   Rationale: True black has ZERO color tint. Even dark green retains some color.
   It's better to mis-classify black as green than green as black.
   
   LIGHTING VARIATIONS HANDLING:
   - Dim lighting: Colors may appear darker but retain their hue
   - Bright lighting: Colors may appear washed out but still distinguishable
   - Poor quality: Focus on RELATIVE comparison between cards
   - Compression artifacts: Compare multiple cards to identify patterns
   
   CONFIDENCE SCORING (Simplified):
   - Clear color visible, matches one family strongly: 0.9-1.0
   - Color visible but mixed signals: 0.7-0.85
   - Borderline case (dark green vs. black): 0.6-0.75
   - Very unclear, poor quality: 0.4-0.6 (flag for review)
   
   DEBUGGING GUIDELINES:
   - If detecting many spades and no clubs → Re-check dark cards for green tint
   - If detecting many clubs and no spades → Re-check if cards are truly colored
   - If suit distribution seems wrong → Verify deck type detection was correct
   - Log deck type and color detection method used for each hand
    ...
    while (attempt < MODEL_ORDER.length) {
      ...
      for (const includeRef of [true, false]) {
        ...
        try {
          ...
          // Log deck type detection and card detection summary
          const allCards = [
          ...
          if (allCards.length > 0) {
            const avgConfidence = allCards.reduce((sum, c) => sum + (c.confidence || 0), 0) / allCards.length;
            
            console.info('Card detection summary:', {
              totalCards: allCards.length,
              averageConfidence: avgConfidence.toFixed(2),
              suitDistribution: {
                hearts: allCards.filter(c => c.suit === 'h').length,
                diamonds: allCards.filter(c => c.suit === 'd').length,
                spades: allCards.filter(c => c.suit === 's').length,
                clubs: allCards.filter(c => c.suit === 'c').length
              },
              note: avgConfidence < 0.8 ? 'Lower confidence may indicate four-color deck or difficult lighting' : 'Standard detection'
            });
            
            // Warn about potential black/green confusion
            ...
            if ((spadeCount === 0 && clubCount >= 4) || (clubCount === 0 && spadeCount >= 4)) {
              console.warn('Unusual spade/club distribution detected:', {
                spades: spadeCount,
                clubs: clubCount,
                warning: 'May indicate black vs. green confusion in four-color deck'
              });
              
              analysisResult.metadata.warnings.push(
                `Unusual suit distribution: ${spadeCount} spades, ${clubCount} clubs. ` +
                `Please verify black/green suit detection if using a four-color deck.`
              );
            }
