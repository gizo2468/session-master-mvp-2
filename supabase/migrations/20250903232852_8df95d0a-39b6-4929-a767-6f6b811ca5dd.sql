-- Fix the get_user_session_statistics function to resolve data sync issues
-- This addresses the critical problem where My Finance shows all zeros while Sessions Stats shows actual data

-- First, let's create a more robust and unified statistics calculation function
CREATE OR REPLACE FUNCTION public.get_unified_session_statistics(
  p_user_id uuid,
...
RETURNS TABLE(
  scope text,
  net_result numeric,
  net_hourly_rate numeric,
  average_net_result numeric,
  total_buy_ins numeric,
  ...
  average_bb100 numeric,
  final_tables integer,
  first_place_finish integer
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Return unified statistics with proper currency handling
  RETURN QUERY
  WITH session_data AS (
    ...
    FROM sessions s
    WHERE s.user_id = p_user_id -- Use the passed user ID explicitly
      ...
      -- Make currency filter optional - only apply if specifically requested and not null
  ...
  table_data AS (
    ...
    WHERE st.user_id = p_user_id -- Use the passed user ID explicitly
  ...
  hands_data AS (
    ...
    WHERE shn.user_id = p_user_id -- Use the passed user ID explicitly
  ...
  -- Calculate statistics based on requested format
  SELECT 
    CASE 
    ...
    -- BB/100 calculation for cash games only
    CASE 
