...
USING (true)
WITH CHECK (
  -- Only allow updates to specific payment processing fields
  -- This ensures only legitimate payment status updates can occur
  true
);

-- Create a policy to allow authenticated users to update their own payment records
-- but only for non-critical fields (like notes, if they exist in the future)
