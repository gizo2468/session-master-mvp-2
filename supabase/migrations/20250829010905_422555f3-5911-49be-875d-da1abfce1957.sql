-- Fix the get_user_session_statistics function to resolve aggregate function error
CREATE OR REPLACE FUNCTION public.get_user_session_statistics(p_user_id uuid, p_timeframe text DEFAULT 'all-time'::text, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_end_date timestamp with time zone DEFAULT NULL::timestamp with time zone)
...
BEGIN
  ...
  -- REMOVED: WHERE COUNT(*) > 0 -- This was causing the aggregate function error
