CREATE OR REPLACE FUNCTION public.get_unified_session_statistics(p_user_id uuid, p_format text DEFAULT 'all'::text, p_timeframe text DEFAULT 'all-time'::text, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_end_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_currency text DEFAULT NULL::text)
...
BEGIN
  ...
  CASE p_timeframe
    ...
    WHEN 'last-month' THEN
      date_filter_start := date_trunc('month', now() - interval '1 month');
      date_filter_end := date_trunc('month', now()) - interval '1 day';
    WHEN 'last-30-days' THEN
      date_filter_start := now() - interval '30 days';
      date_filter_end := now();
    WHEN 'last-7-days' THEN
      date_filter_start := now() - interval '7 days';
      date_filter_end := now();
    WHEN 'this-week' THEN
      date_filter_start := date_trunc('week', now());
      date_filter_end := now();
    WHEN 'last-week' THEN
      date_filter_start := date_trunc('week', now() - interval '1 week');
      date_filter_end := date_trunc('week', now()) - interval '1 day';
    WHEN 'this-year' THEN
      date_filter_start := date_trunc('year', now());
      date_filter_end := now();
    WHEN 'last-year' THEN
      date_filter_start := date_trunc('year', now() - interval '1 year');
      date_filter_end := date_trunc('year', now()) - interval '1 day';
    WHEN 'custom' THEN
  ...
  WITH session_data AS (
    SELECT 
      ...
      CASE 
        ...
        ELSE 'cash'
      END as session_type,
      -- FIXED: Prioritize session_duration (seconds) over timestamp calculation
      CASE 
    ...
    WHERE s.user_id = p_user_id
      ...
      AND (p_currency IS NULL OR p_currency = '' OR s.currency = p_currency OR s.currency IS NULL)
  ),
  ...
  table_data AS (
    ...
    WHERE st.user_id = p_user_id
      AND EXISTS (SELECT 1 FROM session_data sd WHERE sd.id = st.session_id)
  ...
  hands_data AS (
    ...
    WHERE shn.user_id = p_user_id
      AND EXISTS (SELECT 1 FROM session_data sd WHERE sd.id = shn.session_id)
    GROUP BY shn.session_id
  ),

  -- FIXED: Also get bulk hands_played from session_results
  session_results_hands AS (
    SELECT
      sr.session_id,
      COALESCE(sr.hands_played, 0) as bulk_hands
    FROM session_results sr
    WHERE EXISTS (SELECT 1 FROM session_data sd WHERE sd.id = sr.session_id)
  ),
  
  combined_stats AS (
    SELECT 
      sd.*,
      COALESCE(td.table_count, 1) as table_count,
      ...
      COALESCE(td.total_table_payouts, sd.cash_out) - COALESCE(td.total_table_buy_ins, sd.buy_in + COALESCE(sd.rebuy_amount, 0)) as session_profit,
      COALESCE(td.final_table_count, 0) as final_tables_count,
      COALESCE(td.first_place_count, 0) as first_places,
      -- Use the greater of individually logged hands vs bulk hands_played
      GREATEST(COALESCE(hd.hand_count, 0), COALESCE(srh.bulk_hands, 0)) as hands
    FROM session_data sd
    LEFT JOIN table_data td ON sd.id = td.session_id
    LEFT JOIN hands_data hd ON sd.id = hd.session_id
    LEFT JOIN session_results_hands srh ON sd.id = srh.session_id
  )
  
  SELECT 
    CASE 
      WHEN p_format = 'cash' THEN 'cash'
      WHEN p_format = 'tournament' THEN 'tournaments'
      ELSE 'all'
    END::text as scope,
    ...
    CASE 
      WHEN p_format = 'cash' AND SUM(cs.hands) > 0 AND AVG(cs.big_blind) > 0
      THEN (SUM(cs.session_profit) / AVG(cs.big_blind) / SUM(cs.hands)) * 100
      ELSE 0 
    END as average_bb100,
    COALESCE(SUM(cs.final_tables_count), 0)::integer as final_tables,
    COALESCE(SUM(cs.first_places), 0)::integer as first_place_finish
  FROM combined_stats cs
  WHERE (p_format = 'all' OR cs.session_type = p_format);
END;
$function$;