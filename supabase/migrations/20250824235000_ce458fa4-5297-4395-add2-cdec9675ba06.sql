...
DROP FUNCTION IF EXISTS get_user_session_statistics(uuid, text);

-- Create comprehensive user session statistics function
...
BEGIN
  ...
  -- Return statistics for all three scopes
  RETURN QUERY
...
CREATE INDEX IF NOT EXISTS idx_sessions_user_time_format ON sessions(user_id, start_time, end_time, format) WHERE end_time IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_session_tables_session_id ON session_tables(session_id);
CREATE INDEX IF NOT EXISTS idx_session_hands_new_session_id ON session_hands_new(session_id);

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION get_user_session_statistics(uuid, text, timestamptz, timestamptz) TO authenticated;