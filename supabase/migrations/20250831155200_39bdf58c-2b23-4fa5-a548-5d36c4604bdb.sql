-- Fix currency filtering to be optional in get_user_session_statistics function
CREATE OR REPLACE FUNCTION public.get_user_session_statistics(p_user_id uuid, p_timeframe text DEFAULT 'all-time'::text, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_end_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_currency text DEFAULT NULL::text)
...
BEGIN
  ...
  -- Return statistics for all three scopes with optional currency filtering
  RETURN QUERY
  WITH session_data AS (
    ...
    WHERE s.user_id = p_user_id
      ...
      AND (p_currency IS NULL OR s.currency = p_currency) -- Optional currency filter
  ),
