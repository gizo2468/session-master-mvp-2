...
create policy "Coach or Player can view goals" on public.player_goals
for select using (auth.uid() = coach_id or auth.uid() = student_id);

-- Coach can create goals for approved students
create policy "Coach can create goals for approved students" on public.player_goals
...
create policy "Coach can update their goals" on public.player_goals
...
create policy "Student can update own goals" on public.player_goals
for update using (auth.uid() = student_id) with check (student_id = auth.uid());

-- Coach can delete their goals
create policy "Coach can delete goals" on public.player_goals
...
-- Helpful indexes
create index idx_player_goals_student_status_due on public.player_goals (student_id, status, due_date desc);
create index idx_player_goals_coach_created_at on public.player_goals (coach_id, created_at desc);
