-- Fix get_unified_session_statistics to calculate profit from table-level data consistently
-- This fixes the bug where session_profit was calculated from session-level data
-- but buy_ins/payouts were from table-level data, causing mismatches
...
BEGIN
  ...
  combined_stats AS (
    SELECT 
      ...
      -- FIXED: Calculate session_profit from the same data source as buy_ins/payouts
